...
const M1Dashboard = () => {
    ...
    const fetchSensorData = async (range) => {
        try {
            ...
            if (range === 'live') {
                ...
                // Fetch based on the selected time range (5m, 30m, 1h, etc.)
    ...
    useEffect(() => {
        ...
        if (selectedRange === 'live') {
            ...
            // For non-live intervals, refresh the data every minute
            interval = setInterval(() => {
                fetchSensorData(selectedRange);
    ...
    const prepareChartData = () => {
        ...
        const labels = sensorData.map((dataPoint) => {
            ...
            // For "live" data, display time as HH:MM:SS
            if (selectedRange === 'live') {
                const seconds = date.getSeconds().toString().padStart(2, '0');
                return `${hours}:${minutes}:${seconds}`;
            } else {
                // For other ranges, display time as HH:MM
